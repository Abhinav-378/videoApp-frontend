      ...
      <div>
        {
          loading && (
            // display 9 skeletons
            <div className='flex flex-row flex-wrap items-center justify-center w-full mt-5'>
              {Array.from({ length: 9 }).map((_, index) => (
                <div className='gap-1 py-4 md:px-4' key={index}>
                  <VideoCardSkeleton />
                </div>
              ))}
            </div>
          ...
          <div className='flex flex-col md:flex-row flex-wrap items-start justify-start w-full mt-5'>
            {
              videos.map((video) => (
                <div className='flex flex-col w-full md:w-1/3 justify-start items-start gap-1 py-4 md:px-4  ' key={video._id}>
                  <Link className='w-full' to={`/watch/${video._id}`} key={video._id}>
